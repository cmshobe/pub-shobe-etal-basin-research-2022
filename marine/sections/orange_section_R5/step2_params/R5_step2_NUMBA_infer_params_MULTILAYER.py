...
run_number = 'ml2' #used to name results file
n_procs = 40 #number of realizations that can run at once; should match qsub request n_nosed * ppn
...
bedrock_file = '/scratch/cs00048/marine/sections/orange_section_R5/prepro/bedrock_elev_array.npy'
br = np.load(bedrock_file)
bedrock_elev_array = xr.DataArray(br, dims=['time', 'x'])
initial_bedrock = bedrock_elev_array[0, :]

qs_file = '/scratch/cs00048/marine/sections/orange_section_R5/prepro/qs_array.npy'
...
in_ds = xs.create_setup(
			...
			master_clock='time',
			input_vars={
				'grid': {'length': 770000., 'spacing': 10000.},
...
def model(parameter):
    ...
    ds_out.profile__br[-1, 1:]), axis=0), "params_for_record":[travel_dist, k_factor, k_depth, s_crit]}    
...
def distance(x, y): #x is simulated, y is observed
	...
	misfit = np.sqrt((1/(76 * 8)) * np.sum(np.power(y["data"] - x_compacted, 2)/np.power(10,2)))
	...
	with open('/scratch/cs00048/marine/sections/orange_section_R5/step2_params/all_params_' + str(run_number).zfill(3) + '.csv','a') as file:
		file.write(string) 
...
sed_surfaces = np.load('/scratch/cs00048/marine/sections/orange_section_R5/prepro/all_surfaces.npy')
...
return_df.to_csv('/scratch/cs00048/marine/sections/orange_section_R5/step2_params/step2_results_' + str(run_number).zfill(3) + '.csv')
...
best_fit_params = np.array([np.float64(best_fit_travel_dist.iloc[0]), 
                           np.float64(best_fit_k.iloc[0]), 
                           np.float64(best_fit_depth_scale.iloc[0]), 
                           np.float64(best_fit_s_crit.iloc[0])]
                          )
np.save('/scratch/cs00048/marine/sections/orange_section_R5/step2_params/best_fit_params_' + str(run_number).zfill(3), best_fit_params)

...
run_number = 'ld8' #used to name results file
...
erode__k_depth_scale_min = 250
erode__k_depth_scale_max = 39750
...
def model(parameter):
    ...
    return {"data": ds_out.profile__h[-1, 1:], "params_for_record":[k_factor, k_depth]}
...
def distance(x, y): #x is simulated, y is observed
    misfit = np.sqrt((1/66) * np.sum(np.power(y["data"] - x["data"], 2)/np.power(10,2)))

##################################
#INVERSION SCRIPT FOR ORANGE BASIN SECTION R6
...
bedrock_file = '/scratch/cs00048/marine/sections/orange_section_R6/prepro/bedrock_elev_array.npy'
br = np.load(bedrock_file)
bedrock_elev_array = xr.DataArray(br, dims=['time', 'x'])
initial_bedrock = bedrock_elev_array[0, :]

qs_file = '/scratch/cs00048/marine/sections/orange_section_R6/prepro/qs_array.npy'
...
in_ds = xs.create_setup(
			...
			master_clock='time',
			input_vars={
				'grid': {'length': 670000., 'spacing': 10000.},
...
def distance(x, y): #x is simulated, y is observed
    ...
    #calculate misfit
	misfit = np.sqrt((1/(66 * 8)) * np.sum(np.power(y["data"] - x_compacted, 2)/np.power(10,2)))
	...
	with open('/scratch/cs00048/marine/sections/orange_section_R6/step2_params/all_params_' + str(run_number).zfill(3) + '.csv','a') as file:
		file.write(string) 
...
sed_surfaces = np.load('/scratch/cs00048/marine/sections/orange_section_R6/prepro/all_surfaces.npy')
...
return_df.to_csv('/scratch/cs00048/marine/sections/orange_section_R6/step2_params/step2_results_' + str(run_number).zfill(3) + '.csv')
...
np.save('/scratch/cs00048/marine/sections/orange_section_R6/step2_params/best_fit_params_' + str(run_number).zfill(3), best_fit_params)

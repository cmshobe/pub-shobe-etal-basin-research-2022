...
#########################################
#PREPROCESSING SCRIPT FOR ORANGE BASIN SECTION R6
...
def scale_surface(x_imported, y_imported):
    ...
    #of the section.
    max_drawn_x = 804.852  #true length of the section for scaling
    y_imported = -y_imported #[::-1]
    x_new = -(x_imported / max_drawn_x) * section_max_x + section_max_x
    y_new = y_imported + abs(min(y_imported))
    y_new = (y_new / 47.315) * 6643 #rescale sections using measurements from vector editor
    y_new -= max(y_new)
...
folder = '/users/cs00048/marine/sections/orange_section_R6/prepro/strat_data_lines_EDIT/'
...
np.save('/scratch/cs00048/marine/sections/orange_section_R6/prepro/bedrock_elev_array.npy', br_elev_array)
...
#save out array as npy binary
np.save('/scratch/cs00048/marine/sections/orange_section_R6/prepro/qs_array.npy', qs_array)
...
np.save('/scratch/cs00048/marine/sections/orange_section_R6/prepro/all_surfaces.npy', all_surfaces)
